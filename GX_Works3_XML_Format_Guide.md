...
### CSV标签导入失败（中文注释/标签批量导入）

CSV导入以GX Works3实际导出并验证可导入的`Motor_Control_test1.csv`为准：
1. **编码**：必须为`UTF-16`（带BOM）
2. **分隔符**：必须为`TAB`（制表符），不是逗号
3. **引号**：每个字段必须使用英文双引号`"`包裹
4. **全局/局部分开**：
   - 全局CSV第1行：`"(工程未设置)"`
   - 局部CSV第1行：`"(ProgramName)"`（例如`"(Motor_Control)"`）
5. **标签名必须精确匹配工程**：例如工程内是`gStart_Button`，CSV也必须是`gStart_Button`
